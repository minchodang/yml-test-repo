...
이 component는 선택 사항이며 표준 native form과 유사하게 form 제출을 처리합니다.

기본적으로, form 제출 데이터는 [FormData](https://developer.mozilla.org/en-US/docs/Web/API/FormData)로 POST 요청을 보냅니다. FormData를 제출하지 않으려면 `headers` props를 제공하고 대신 `application/json`을 사용할 수 있습니다.
...
| `onError`        | <TypeText>`Function`</TypeText>                    | 서버에 대한 요청 실패 후 호출되는 함수. <br/><br/>`setError` 함수는 오류 상태를 업데이트하기 위해 호출됩니다. `root.server`가 에러 키로 사용됩니다. | <CodeArea withOutCopy rawData="<Form onError={({ response }) => {}} />" />                                         |
...
- 제출 데이터를 준비하거나 생략하려면, [`handleSubmit`](/docs/useform/handlesubmit) or `onSubmit`을 사용하세요.
  ```javascript
  ...
  />
  ```
- Progressive Enhancement는 SSR framework에만 적용됩니다.
